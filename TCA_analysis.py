...
if False:
    ...
    #open the sar nc file and have the date
        ...
        try:
            ...
            sar_mask = sar_nc.variables['mask'][:]
            sar_mask_bool = (sar_mask != -1)
            sar_wind.mask = sar_wind.mask | sar_mask_bool
            ...
            if smap_AOI:
                ...
                if cyg_AOI and smap_AOI:
                    ...
                    if plotting:
                        ...
                        ax.set_title(f'SMAP, SAR, & CYGNSS {str(cyg_AOI_time_start)[0:10]}-{str(cyg_AOI_time_end)[11:16]}', loc='right')
                        
                        plt.subplots_adjust(left=0.55, right=0.65, bottom=0.1, top=0.9, wspace=0.5)
                        plt.tight_layout()
                        
                        # # saving file
                        directory = r'C:\Users\{0}\OneDrive - RMIT University\PHD\Plots\TCA\TCA{1}.png'.format(comp,num_events)
                        fig.savefig(directory, format='png', dpi=300, bbox_inches='tight', pad_inches=0)
                        plt.show()
                        
                        TCA_dates.append(str(cyg_AOI_time_start)[0:10])
...
for cyg_version in ['noaa_1.2','storm_centric']:
    ...
    latitudes = np.asarray([item[3] for item in cleaned_array])
    longitudes = np.asarray([item[4] for item in cleaned_array])
    times = np.asarray([item[5] for item in cleaned_array])
    years = (times.astype('datetime64[s]').astype('datetime64[Y]').astype(int) + 1970)
    unique_years, counts = np.unique(years, return_counts=True)
    # Print the number of measurements in each basin
    for year, count in zip(unique_years, counts):
        print(f"{year}: {count}")
        index = np.where(years == year)
        # print(str(year) + cyg_version+' SAR ' + str(comparison(cyg_vals[index],sar_vals[index])))
        # print(str(year) + cyg_version+' SMAP ' + str(comparison(cyg_vals[index],smap_vals[index])))
    
    # basin analyse
    # Initialize an empty array for basin categories
    basins = np.empty(latitudes.shape, dtype=object)
    basins[(latitudes < 0) & (latitudes > -60) & ((longitudes > 135) & (longitudes < 180) | (longitudes > -180) & (longitudes < -120))] = 'Southwest Pacific'
    basins[(latitudes < 0) & (latitudes > -60) & (longitudes > 90) & (longitudes < 135)] = 'Southeast Indian'
    basins[(latitudes < 0) & (latitudes > -60) & (longitudes > 30) & (longitudes < 90)] = 'Southwest Indian'
    basins[(latitudes > 0) & (latitudes < 60) & (longitudes > 100) & (longitudes < 180)] = 'Northwest Pacific'
    basins[(latitudes > 0) & (latitudes < 60) & (longitudes > -100) & (longitudes < -10)] = 'North Atlantic'
    basins[(latitudes > 0) & (latitudes < 60) & (longitudes > -160) & (longitudes < -100)] = 'Northeast Pacific'
    basins[(latitudes > 0) & (latitudes < 60) & (longitudes > 45) & (longitudes < 100)] = 'Northern Indian'
    unique_basins, counts = np.unique(basins, return_counts=True)
    
    # Print the number of measurements in each basin
    for basin, count in zip(unique_basins, counts):
        print(f"{basin}: {count}")
        index = np.where(basins == basin)
        # print(str(basin) + cyg_version+' SAR ' + str(comparison(cyg_vals[index],sar_vals[index])))
        # print(str(basin) + cyg_version+' SMAP ' + str(comparison(cyg_vals[index],smap_vals[index])))
    
    # If a point doesn't fit any basin, mark it as "Unknown"
    basins[basins == np.array(None)] = 'Unknown'
    
    save_name = '\TCA_'+ cyg_version
    save_dir = r'C:\Users\\'+comp+'\OneDrive - RMIT University\PHD\Plots' 
    save_location = save_dir + save_name
    # density_plot(sar_vals, 'SAR', cyg_vals, 'CYGNSS', save_location +'SAR_CYG.png',0,51,110,False)
    density_plot(sar_vals, 'SAR', smap_vals, 'SMAP', save_location +'SAR_SMAP.png',0,51,110,False)
    # density_plot(smap_vals, 'SMAP',cyg_vals, 'CYGNSS', save_location +'SMAP_CYG.png',0,51,110, False)
    density_plot((smap_vals+sar_vals)/2, 'SMAP SAR Avg.',cyg_vals, 'CYGNSS', save_location +'SMAPSAR_CYG.png',0,51,110, False)

    
    threshold = 17
    index = np.where(smap_vals<threshold)
    index_above = np.where(smap_vals >= threshold)
    cyg_below =cyg_vals[index] ; cyg_above = cyg_vals[index_above]
    sar_below = sar_vals[index] ; sar_above = sar_vals[index_above]
    smap_below = smap_vals[index] ; smap_above = smap_vals[index_above]
    

    print(cyg_version+'Above SAR ' + str(comparison(cyg_above,sar_above)))
    print(cyg_version+'Above SMAP ' + str(comparison(cyg_above,smap_above)))
    print(cyg_version+'below SAR ' + str(comparison(cyg_below,sar_below)))
    print(cyg_version+'below SMAP ' + str(comparison(cyg_below,smap_below)))
    # print(np.var(sar_above-cyg_above),np.var(smap_above-cyg_above))
    # print(np.var(sar_below-cyg_below),np.var(smap_below-cyg_below))
    
    for error_type in ['add','multi']:
        ...
        print(cyg_version, error_type, ubRMSE, RMSE, cross_corr)
        print(cyg_version, error_type, ubRMSE_CI[:,2],RMSE_CI[:,2],corr_CI[:,2])
