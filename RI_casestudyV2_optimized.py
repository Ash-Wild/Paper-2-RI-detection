# -*- coding: utf-8 -*-
"""
RI Case Study Analysis V2 - OPTIMIZED
Performs detailed case study analysis on selected TC events for YSLF, Merged, and NOAA compared to IBTrACS reference.
OPTIMIZATIONS: Pre-filtered files, early filtering, parallelization, float32 precision, vectorized track matching
...
def _extract_box_float_var(cached_vars, varname, box_indices):
    ...
    if getattr(var_full, 'size', 0) == 0:
        return np.array([])
    try:
        selected = np.asarray(np.ma.filled(var_full[box_indices], np.nan), dtype=np.float32)
    except Exception:
        return np.array([])
    return selected
...
def filter_files_by_date(file_list, date_list):
    """OPTIMIZATION: Pre-filter files to only those overlapping with date range."""
    filtered_files = []
...
def process_case_study(case_study, time, storm_id, vmax, latitude, longitude, ri_rd, durations,
                      intensities, index_after, index_before, noaa_like_files_map, date_to_noaa_idx,
                      merged_cyg_files_list, merged_file_dates, cyg_files_num,
                      output_dir, comp):
    """Process a single case study event."""
    ...
    # Vectorized interpolation using float32
    tc_times_s = tc_time.astype('datetime64[s]').astype('int64')
    interp_s = tc_time_interpolated.astype('datetime64[s]').astype('int64')

    tc_vmax_vals = np.ma.filled(tc_vmax, np.nan).astype(np.float32)
    tc_lat_vals = np.asarray(tc_latitude).astype(np.float32)
    tc_lon_vals = np.asarray(tc_longitude).astype(np.float32)
    ...
    # OPTIMIZATION 1: PRE-FILTER FILES BY DATE
    print(f"  {case_study[0]}: Pre-filtering files...")
    filtered_noaa_files = {}
    for source_name in noaa_like_files_map:
        all_files = noaa_like_files_map[source_name]
        filtered_noaa_files[source_name] = filter_files_by_date(all_files, date_list)
    ...
    print(f"  {case_study[0]}: Loading NOAA/L2 data...")
    noaa_cache = {source_name: {} for source_name in noaa_like_files_map}

    for source_name in noaa_like_files_map:
        source_files = filtered_noaa_files[source_name]  # Use pre-filtered
        source_cache = noaa_cache.get(source_name, {})

        for cyg_file_path in source_files:
            cyg_file_path_str = str(cyg_file_path)

            if cyg_file_path_str in source_cache:
                cached = source_cache[cyg_file_path_str]
                ...
                try:
                    cyg_nc = nc.Dataset(cyg_file_path_str)
                    longitudes = cyg_nc.variables['lon'][:]
                    cygnss_lats = cyg_nc.variables['lat'][:].astype(np.float32)
                    ...
                    cygnss_wind = cyg_nc.variables[wind_var_name][:].astype(np.float32)
                    ...
                    cygnss_lons = cygnss_lons.astype(np.float32)
                    ...
                    for varname in variables:
                        try:
                            if varname in cyg_nc.variables:
                                vars_cache[varname] = cyg_nc.variables[varname][:].astype(np.float32)
                            else:
                                ...
                                if source_found is not None:
                                    vars_cache[varname] = cyg_nc.variables[source_found][:].astype(np.float32)
                                elif varname in avg_alias_fields:
                                    alias_arrays = []
                                    for alias_name in avg_alias_fields[varname]:
                                        if alias_name in cyg_nc.variables:
                                            alias_vals = np.asarray(np.ma.filled(cyg_nc.variables[alias_name][:], np.nan), dtype=np.float32)
                                    ...
                                    if alias_found is not None:
                                        vars_cache[varname] = cyg_nc.variables[alias_found][:].astype(np.float32)
                                    else:
                    ...
                    source_cache[cyg_file_path_str] = cached
            ...
            # OPTIMIZATION 2: EARLY FILTERING - Box selection first
            ...
            meas_lats = cygnss_lats[box_indices].astype(np.float32)
            meas_lons = cygnss_lons[box_indices].astype(np.float32)
            meas_winds = cygnss_wind[box_indices].astype(np.float32)
            ...
            # Spatiotemporal matching on reduced dataset
            ...
            if len(selected_indices) > 0:
                cyg_winds_vals = np.ma.filled(meas_winds[selected_indices], np.nan).astype(np.float32)
                cyg_tracks_vals = cygnss_tracks[box_indices][selected_indices]

                results_by_source[source_name]['lats'].extend(meas_lats[selected_indices].tolist())
                results_by_source[source_name]['lons'].extend(meas_lons[selected_indices].tolist())
                results_by_source[source_name]['winds'].extend(cyg_winds_vals.tolist())
                results_by_source[source_name]['times'].extend(meas_times[selected_indices].tolist())
    ...
    # Process Merged data
    print(f"  {case_study[0]}: Loading Merged data...")
    if np.any(np.isin(date_list, np.array(merged_file_dates))):
        ...
        for cyg_file_path in unique_storm_files:
            try:
                ...
                cyg_storm_lat = cyg_nc.variables['best_track_storm_center_lat'][:].astype(np.float32)
                cyg_storm_lon = ((cyg_nc.variables['best_track_storm_center_lon'][:] + 180) % 360) - 180
                cyg_storm_lon = cyg_storm_lon.astype(np.float32)
                ...
                if len(epochs) > 1:
                    cygnss_wind = cyg_nc.variables['wind_speed'][epochs].astype(np.float32)
                    ...
                    cyg_vmax = np.full(len(epochs), np.nan, dtype=np.float32)

                    # Extract wind speed at vmax location
                    cygnss_lats = np.asarray(cyg_nc.variables['lat'][:], dtype=np.float32)
                    cygnss_lons = np.asarray(((cyg_nc.variables['lon'][:] + 180) % 360) - 180, dtype=np.float32)
                    vmax_lat = np.asarray(cyg_nc.variables['cygnss_vmax_lat'][epochs], dtype=np.float32)
                    vmax_lon = np.asarray(((cyg_nc.variables['cygnss_vmax_lon'][epochs] + 180) % 360) - 180, dtype=np.float32)
    ...
    print(f"  {case_study[0]}: Analyzing results...")
    ...
    print(f"  {case_study[0]}: Saving measurement metadata...")
    for source_name in ['noaa_1.2', 'l2_3.2', 'merged']:
        if source_name == 'merged':
            if len(results_by_source['merged']['epochs']) > 0:
                ...
                merged_csv = os.path.join(output_dir, f'{tc_storm_id}_Merged_measurements.csv')
            ...
            if len(results_by_source[source_name]['winds']) > 0:
                ...
                meas_csv = os.path.join(output_dir, f'{tc_storm_id}_{product_name}_measurements.csv')
                meas_df.to_csv(meas_csv, index=False)
                print(f"    Saved: {os.path.basename(meas_csv)}")

    # OPTIMIZATION 4: VECTORIZED TRACK MATCHING
    for source_name in ['noaa_1.2', 'l2_3.2']:
        if len(results_by_source[source_name]['winds']) > 0:
            winds = np.array(results_by_source[source_name]['winds'], dtype=np.float32)
            ...
            # Vectorized track comparison
            track_keys = np.array([_track_key(t) for t in tracks], dtype=object)
            ...
            max_wind = np.full(len(unique_tracks), np.nan, dtype=np.float32)
            valid_track_peak = np.zeros(len(unique_tracks), dtype=bool)

            for i, track in enumerate(unique_tracks):
                track_mask = (track_keys == track)  # Vectorized
                track_times = times[track_mask]
                track_winds = winds[track_mask]

                if track_times.size > 0:
                    track_winds_vals = np.asarray(np.ma.filled(track_winds, np.nan), dtype=np.float32)
            ...
            if len(valid_peaks) > 0:
                ...
                if ri_result[0]:
                    ...
                    initial_wind = float(valid_winds[initial_max_ind])
                    final_wind = float(valid_winds[final_max_ind])
    ...
    if len(results_by_source['merged']['epochs']) > 0:
        ...
        summary_data['num_tracks'].append(1)
        ...
        if len(vmax_values) >= 2:
            initial_wind = float(vmax_values[0])
            final_wind = float(vmax_values[-1])
            summary_data['initial_wind'].append(initial_wind)
            summary_data['final_wind'].append(final_wind)
            summary_data['wind_change_estimate'].append(final_wind - initial_wind)
    ...
    summary_df.loc[len(summary_df)] = {
        ...
        'num_measurements': 1,
        'num_tracks': 1,
        'mean_wind': float((ri_vmax_start + ri_vmax_end) / 2),
        'max_wind': float(ri_vmax_end),
        'min_wind': float(ri_vmax_start),
        'initial_wind': float(ri_vmax_start),
        'final_wind': float(ri_vmax_end),
        'wind_change_estimate': float(ri_vmax_change),
        'ri_valid': True
    }

    # Print results
    print(f"\n  {case_study[0]}: Summary - {tc_storm_id}")
    print(f"    Duration: {tc_durations}h | RI Change: {ri_vmax_change:.2f} m/s")
    ...
    output_file = os.path.join(output_dir, f'{tc_storm_id}_casestudy_comparison_optimized.png')
...
print("Loading IBTrACS RI events...")
...
# OPTIMIZATION 3: PARALLEL PROCESSING - Process case studies in parallel
if __name__ == '__main__':
    print(f"\n{'='*80}")
    print(f"OPTIMIZED RI Case Study Analysis V2")
    print(f"Processing {len(case_study_events)} case studies in parallel...")
    print(f"{'='*80}\n")
    ...
    process_func = partial(
        ...
        noaa_like_files_map=noaa_like_files_map,
        date_to_noaa_idx=date_to_noaa_idx,
        merged_cyg_files_list=merged_cyg_files_list,
        merged_file_dates=merged_file_dates,
        cyg_files_num=cyg_files_num,
        output_dir=output_dir,
        comp=comp
    )

    # Use multiprocessing for parallel processing
    num_workers = min(4, len(case_study_events))
    print(f"Using {num_workers} worker(s) for parallel processing\n")

    with Pool(num_workers) as pool:
        results = pool.map(process_func, case_study_events)
