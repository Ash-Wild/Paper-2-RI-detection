...
def adjust_wind(df,gauge_lat,gauge_lon,h):
    import math
    import pandas as pd
    
    df.replace(-9999, math.nan, inplace=True)
    df['julian_date'] = (df['Datetime'] - pd.Timestamp(df['Datetime'].dt.year.iloc[0], 1, 1)).dt.total_seconds() / (24 * 3600)

    len_df = df.shape[0]
    nan_array =  np.full(len_df, math.nan)
    
    u = np.array(df['Wind Gust'])
    t = np.array(df['Air Temperature'])
    rh = np.full(len_df, 70)
    P = np.array(df['Barometric Pressure'])
    ts = np.array(df['Water Temperature'])
    sw_dn = np.full(len_df, 250)
    lw_dn = np.full(len_df, 400)
    lat = gauge_lat
    lon = gauge_lon
    zi = 600
    rain = nan_array
    ...
    zrf_t=10.0;
    zrf_q=10.0;
    Ss = np.full(len_df, 3.5)
    jd = np.array(df['julian_date'])
    cp = nan_array
    sigH = nan_array
...
if __name__ == '__main__':
    ...
    path = '/Users/Ashley/OneDrive - RMIT University/PHD/Data/BOM stations/PSLGM/'
    fil = 'TEST-data.txt'   
    data = np.genfromtxt(path+fil, skip_header=1)
    u = data[:,5]
    t = data[:,3]
    rh = np.full(len(data[:,0]), 70)
    P = data[:,4]
    ts = data[:,2]
    sw_dn = np.full(len(data[:,0]), 250)
    lw_dn = np.full(len(data[:,0]), 400)
    lat = -21.199
    lon = -159.786
    zi = 600
    rain = data[:,13]
    ...
    zrf_t=10.0;
    zrf_q=10.0;
    Ss = np.full(len(data[:,0]), 3.5)
    jd = data[:,6]
    cp = data[:,13]
    sigH = data[:,13]
    
    A=coare36vn_zrf_et(u, zu , t, zt, rh, zq, P, ts, sw_dn, lw_dn, lat, lon,jd, zi,rain, Ss, cp , sigH, zrf_u, zrf_t, zrf_q)
    fnameA = os.path.join(path,'TEST-data-output.txt')
